...
class Appointment(models.Model):
    ...
    date_to = fields.Datetime(string='Date To', copy=False, tracking=1)
    ...
    def _inverse_planned_duration(self):
        for rec in self:
            ...
            # FIXED: Only update date_to if we're not already setting it explicitly
            if rec.date and not rec.date_to:
    ...
    @api.model
    def create_appointment_from_calendar(self, appointment_vals):
        """
        Special method to create appointments from calendar that bypasses all computed fields
        """
        _logger.info("Creating appointment from calendar: %s", appointment_vals)

        # Create the appointment directly
        appointment = self.create(appointment_vals)

        # Double-check the dates are correct
        if 'date_to' in appointment_vals and appointment.date_to != appointment_vals['date_to']:
            _logger.warning("Date_to was changed during creation. Forcing correction.")
            appointment.write({'date_to': appointment_vals['date_to']})

        _logger.info("Final appointment: date=%s, date_to=%s", appointment.date, appointment.date_to)
    ...
    def create(self, vals_list):
        ...
        results = self.env['hms.appointment']
        for values in vals_list:
            # Store the critical values
            original_date_to = values.get('date_to')
            ...
            # Create using ORM
            appointment = super(Appointment, self).create([values])
            results += appointment

            # Check if date_to was changed and force it back if needed
            if original_date_to and appointment.date_to != original_date_to:
                _logger.info("Date_to was changed from %s to %s, forcing back", original_date_to, appointment.date_to)
                # Use direct SQL to bypass any ORM constraints
                self.env.cr.execute("""
                    UPDATE hms_appointment 
                    SET date_to = %s 
                    WHERE id = %s
                """, (original_date_to, appointment.id))
                # Invalidate the cache for this record
                appointment.invalidate_recordset(['date_to'])

            # Run post-create logic
            appointment.update_reminder_dates()
            appointment.update_appoinemtn_refering()

        return results

    # FIXED: Override write method to ensure proper duration handling
    def write(self, values):
        # If date is changed but date_to is not, adjust date_to to maintain duration
        if 'date' in values and 'date_to' not in values:
            for record in self:
                if record.date and record.date_to:
                    original_duration = record.date_to - record.date
                    new_date = fields.Datetime.from_string(values['date'])
                    new_date_to = new_date + original_duration
                    values['date_to'] = fields.Datetime.to_string(new_date_to)

        # If date_to is provided, ensure it's preserved
        result = super(Appointment, self).write(values)
